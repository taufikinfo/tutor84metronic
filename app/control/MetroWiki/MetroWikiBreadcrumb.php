...
class MetroWikiBreadcrumb extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTBreadcrumb')) {
                $classRef = 'KTBreadcrumb';
                // Simple instantiation logic depending on what is supported
                if ('Breadcrumb' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Breadcrumb Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTBreadcrumb is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Breadcrumb Reference');
        $docPanel->getBody()->id = 'wiki-content-breadcrumb';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/breadcrumb.md';
        ...
        TScript::create("
            function renderWiki_breadcrumb() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-breadcrumb').html(marked.parse(content));
                }
            }
            
            if (typeof marked === 'undefined') {
                $.getScript('https://cdn.jsdelivr.net/npm/marked/marked.min.js', function() {
                    renderWiki_breadcrumb();
                });
            } else {
                renderWiki_breadcrumb();
            }
