...
class MetroWikiCarousel extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTCarousel')) {
                ...
                if ('Carousel' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Carousel Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTCarousel is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Carousel Reference');
        $docPanel->getBody()->id = 'wiki-content-carousel';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/carousel.md';
        ...
        TScript::create("
            function renderWiki_carousel() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-carousel').html(marked.parse(content));
                }
            }
