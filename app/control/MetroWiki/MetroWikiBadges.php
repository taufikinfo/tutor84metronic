...
class MetroWikiBadges extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTBadges')) {
                ...
                if ('Badges' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Badges Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTBadges is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Badges Reference');
        $docPanel->getBody()->id = 'wiki-content-badges';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/badges.md';
        ...
        TScript::create("
            function renderWiki_badges() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-badges').html(marked.parse(content));
                }
            }
