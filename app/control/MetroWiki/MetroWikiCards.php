...
class MetroWikiCards extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTCards')) {
                ...
                if ('Cards' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Cards Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTCards is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Cards Reference');
        $docPanel->getBody()->id = 'wiki-content-cards';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/cards.md';
        ...
        TScript::create("
            function renderWiki_cards() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-cards').html(marked.parse(content));
                }
            }
