...
class MetroWikiForms extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTForms')) {
                ...
                if ('Forms' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Forms Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTForms is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Forms Reference');
        $docPanel->getBody()->id = 'wiki-content-forms';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/forms.md';
        ...
        TScript::create("
            function renderWiki_forms() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-forms').html(marked.parse(content));
                }
            }
