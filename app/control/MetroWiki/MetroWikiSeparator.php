...
class MetroWikiSeparator extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTSeparator')) {
                $classRef = 'KTSeparator';
                // Simple instantiation logic depending on what is supported
                if ('Separator' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Separator Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTSeparator is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Separator Reference');
        $docPanel->getBody()->id = 'wiki-content-separator';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/separator.md';
        ...
        TScript::create("
            function renderWiki_separator() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-separator').html(marked.parse(content));
                }
            }
            
            if (typeof marked === 'undefined') {
                $.getScript('https://cdn.jsdelivr.net/npm/marked/marked.min.js', function() {
                    renderWiki_separator();
                });
            } else {
                renderWiki_separator();
            }
