...
class MetroWikiTables extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTTables')) {
                ...
                if ('Tables' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Tables Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTTables is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Tables Reference');
        $docPanel->getBody()->id = 'wiki-content-tables';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/tables.md';
        ...
        TScript::create("
            function renderWiki_tables() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-tables').html(marked.parse(content));
                }
            }
