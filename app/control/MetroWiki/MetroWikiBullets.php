...
class MetroWikiBullets extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTBullets')) {
                ...
                if ('Bullets' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Bullets Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTBullets is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Bullets Reference');
        $docPanel->getBody()->id = 'wiki-content-bullets';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/bullets.md';
        ...
        TScript::create("
            function renderWiki_bullets() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-bullets').html(marked.parse(content));
                }
            }
