...
class MetroWikiHover extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTHover')) {
                ...
                if ('Hover' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Hover Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTHover is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Hover Reference');
        $docPanel->getBody()->id = 'wiki-content-hover';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/hover.md';
        ...
        TScript::create("
            function renderWiki_hover() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-hover').html(marked.parse(content));
                }
            }
