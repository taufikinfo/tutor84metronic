...
class MetroWikiPageLoading extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTPageLoading')) {
                $classRef = 'KTPageLoading';
                // Simple instantiation logic depending on what is supported
                if ('PageLoading' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live PageLoading Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTPageLoading is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: PageLoading Reference');
        $docPanel->getBody()->id = 'wiki-content-pageloading';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/pageloading.md';
        ...
        TScript::create("
            function renderWiki_pageloading() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-pageloading').html(marked.parse(content));
                }
            }
            
            if (typeof marked === 'undefined') {
                $.getScript('https://cdn.jsdelivr.net/npm/marked/marked.min.js', function() {
                    renderWiki_pageloading();
                });
            } else {
                renderWiki_pageloading();
            }
