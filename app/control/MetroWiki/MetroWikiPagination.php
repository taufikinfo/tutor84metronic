...
class MetroWikiPagination extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTPagination')) {
                $classRef = 'KTPagination';
                ...
                if ('Pagination' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Pagination Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTPagination is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Pagination Reference');
        $docPanel->getBody()->id = 'wiki-content-pagination';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/pagination.md';
        ...
        TScript::create("
            function renderWiki_pagination() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-pagination').html(marked.parse(content));
                }
            }
            
            if (typeof marked === 'undefined') {
                $.getScript('https://cdn.jsdelivr.net/npm/marked/marked.min.js', function() {
                    renderWiki_pagination();
                });
            } else {
                renderWiki_pagination();
            }
