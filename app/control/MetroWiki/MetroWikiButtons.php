...
class MetroWikiButtons extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTButtons')) {
                ...
                if ('Buttons' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Buttons Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTButtons is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Buttons Reference');
        $docPanel->getBody()->id = 'wiki-content-buttons';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/buttons.md';
        ...
        TScript::create("
            function renderWiki_buttons() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-buttons').html(marked.parse(content));
                }
            }
