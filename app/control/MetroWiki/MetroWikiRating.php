...
class MetroWikiRating extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTRating')) {
                ...
                if ('Rating' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Rating Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTRating is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Rating Reference');
        $docPanel->getBody()->id = 'wiki-content-rating';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/rating.md';
        ...
        TScript::create("
            function renderWiki_rating() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-rating').html(marked.parse(content));
                }
            }
