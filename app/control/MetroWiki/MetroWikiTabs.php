...
class MetroWikiTabs extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTTabs')) {
                ...
                if ('Tabs' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Tabs Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTTabs is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        TScript::create("
            function renderWiki_tabs() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-tabs').html(marked.parse(content));
                }
            }
