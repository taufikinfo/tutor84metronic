...
class MetroWikiOverlay extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTOverlay')) {
                ...
                if ('Overlay' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Overlay Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTOverlay is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Overlay Reference');
        $docPanel->getBody()->id = 'wiki-content-overlay';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/overlay.md';
        ...
        TScript::create("
            function renderWiki_overlay() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-overlay').html(marked.parse(content));
                }
            }
