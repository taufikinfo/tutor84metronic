...
class MetroWikiUnderline extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTUnderline')) {
                $classRef = 'KTUnderline';
                // Simple instantiation logic depending on what is supported
                if ('Underline' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Underline Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTUnderline is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Underline Reference');
        $docPanel->getBody()->id = 'wiki-content-underline';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/underline.md';
        ...
        TScript::create("
            function renderWiki_underline() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-underline').html(marked.parse(content));
                }
            }
            
            if (typeof marked === 'undefined') {
                $.getScript('https://cdn.jsdelivr.net/npm/marked/marked.min.js', function() {
                    renderWiki_underline();
                });
            } else {
                renderWiki_underline();
            }
