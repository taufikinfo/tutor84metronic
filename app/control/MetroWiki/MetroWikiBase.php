...
class MetroWikiBase extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTBase')) {
                ...
                if ('Base' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Base Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTBase is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        TScript::create("
            function renderWiki_base() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-base').html(marked.parse(content));
                }
            }
