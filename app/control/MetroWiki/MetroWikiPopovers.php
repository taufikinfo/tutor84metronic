...
class MetroWikiPopovers extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTPopovers')) {
                ...
                if ('Popovers' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Popovers Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTPopovers is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Popovers Reference');
        $docPanel->getBody()->id = 'wiki-content-popovers';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/popovers.md';
        ...
        TScript::create("
            function renderWiki_popovers() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-popovers').html(marked.parse(content));
                }
            }
