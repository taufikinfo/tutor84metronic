...
class MetroWikiModal extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTModal')) {
                ...
                if ('Modal' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Modal Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTModal is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Modal Reference');
        $docPanel->getBody()->id = 'wiki-content-modal';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/modal.md';
        ...
        TScript::create("
            function renderWiki_modal() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-modal').html(marked.parse(content));
                }
            }
