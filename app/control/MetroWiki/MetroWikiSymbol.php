...
class MetroWikiSymbol extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTSymbol')) {
                ...
                if ('Symbol' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Symbol Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTSymbol is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Symbol Reference');
        $docPanel->getBody()->id = 'wiki-content-symbol';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/symbol.md';
        ...
        TScript::create("
            function renderWiki_symbol() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-symbol').html(marked.parse(content));
                }
            }
