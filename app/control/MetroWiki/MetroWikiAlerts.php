...
class MetroWikiAlerts extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTAlerts')) {
                ...
                if ('Alerts' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Alerts Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTAlerts is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Alerts Reference');
        $docPanel->getBody()->id = 'wiki-content-alerts';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/alerts.md';
        ...
        TScript::create("
            function renderWiki_alerts() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-alerts').html(marked.parse(content));
                }
            }
