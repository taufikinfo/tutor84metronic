...
class MetroWikiToasts extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTToasts')) {
                ...
                if ('Toasts' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Toasts Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTToasts is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Toasts Reference');
        $docPanel->getBody()->id = 'wiki-content-toasts';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/toasts.md';
        ...
        TScript::create("
            function renderWiki_toasts() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-toasts').html(marked.parse(content));
                }
            }
