...
class MetroWikiHelpers extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTHelpers')) {
                ...
                if ('Helpers' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Helpers Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTHelpers is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Helpers Reference');
        $docPanel->getBody()->id = 'wiki-content-helpers';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/helpers.md';
        ...
        TScript::create("
            function renderWiki_helpers() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-helpers').html(marked.parse(content));
                }
            }
