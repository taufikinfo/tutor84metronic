...
class MetroWikiUtilities extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTUtilities')) {
                $classRef = 'KTUtilities';
                // Simple instantiation logic depending on what is supported
                if ('Utilities' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Utilities Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTUtilities is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Utilities Reference');
        $docPanel->getBody()->id = 'wiki-content-utilities';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/utilities.md';
        ...
        TScript::create("
            function renderWiki_utilities() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-utilities').html(marked.parse(content));
                }
            }
            
            if (typeof marked === 'undefined') {
                $.getScript('https://cdn.jsdelivr.net/npm/marked/marked.min.js', function() {
                    renderWiki_utilities();
                });
            } else {
                renderWiki_utilities();
            }
