...
class MetroWikiRotate extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTRotate')) {
                ...
                if ('Rotate' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Rotate Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTRotate is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Rotate Reference');
        $docPanel->getBody()->id = 'wiki-content-rotate';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/rotate.md';
        ...
        TScript::create("
            function renderWiki_rotate() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-rotate').html(marked.parse(content));
                }
            }
