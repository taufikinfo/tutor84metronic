...
class MetroWikiIndicator extends TPage
{
    public function __construct()
    {
        ...
        try {
            if (class_exists('KTIndicator')) {
                $classRef = 'KTIndicator';
                ...
                if ('Indicator' === 'Buttons') {
                    ...
                    $demo = $classRef::make();
                    $demo->add("<i>Live Indicator Component</i>");
                    $showcasePanel->add($demo->render());
                }
            } else {
                $showcasePanel->add("<i>Component Wrapper KTIndicator is actively under development or does not have a visual representation here.</i>");
            }
        } catch (Exception $e) {
        ...
        // --- THE DOCUMENTATION ---
        $docPanel = new TPanelGroup('Metro Wiki: Indicator Reference');
        $docPanel->getBody()->id = 'wiki-content-indicator';
        $docPanel->getBody()->style = 'padding: 2rem; background: var(--bs-body-bg); border-radius: 0.5rem; border: 1px solid var(--bs-gray-300);';
        
        $filepath = 'docs/wiki/indicator.md';
        ...
        TScript::create("
            function renderWiki_indicator() {
                var content = " . json_encode($md) . ";
                if (typeof marked !== 'undefined') {
                    $('#wiki-content-indicator').html(marked.parse(content));
                }
            }
            
            if (typeof marked === 'undefined') {
                $.getScript('https://cdn.jsdelivr.net/npm/marked/marked.min.js', function() {
                    renderWiki_indicator();
                });
            } else {
                renderWiki_indicator();
            }
