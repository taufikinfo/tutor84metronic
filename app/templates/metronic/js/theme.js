 ...
 */
Template.convertMetronicMenu = function() {
    var $menu = $('#side-menu');
    if ($menu.length === 0) return;
    
    $menu.addClass('menu menu-column menu-rounded menu-sub-indention px-3').attr('id', 'kt_app_sidebar_menu').attr('data-kt-menu', 'true');
    $menu.removeClass('sidebar-nav');
    
    $menu.find('li.sidebar-item').removeClass('sidebar-item').addClass('menu-item');
    $menu.find('a.sidebar-link').removeClass('sidebar-link collapsed').addClass('menu-link').each(function() {
        let $icon = $(this).children('i, svg').first();
        if ($icon.length > 0) {
            $icon.wrap('<span class="menu-icon"></span>');
        } else {
            $(this).prepend('<span class="menu-bullet"><span class="bullet bullet-dot"></span></span>');
        }
        
        let $span = $(this).children('span').not('.menu-icon, .menu-bullet, .menu-arrow');
        if ($span.length > 0) {
            $span.addClass('menu-title');
        }
    });
    
    $menu.find('ul.sidebar-dropdown').removeClass('sidebar-dropdown list-unstyled collapse level-1 level-2 level-3').addClass('menu-sub menu-sub-accordion');
    
    $menu.find('li.menu-item').each(function() {
        if ($(this).children('ul.menu-sub').length > 0) {
            $(this).attr('data-kt-menu-trigger', 'click');
            $(this).children('a.menu-link').append('<span class="menu-arrow"></span>');
            $(this).children('a.menu-link').removeAttr('data-bs-toggle').removeAttr('data-bs-target');
        }
    });
    
    $menu.find('li.menu-item.active').removeClass('active').addClass('here show');
    $menu.find('a.menu-link.active').addClass('active');

    // Re-initialize the Metronic Menu component dynamically since the DOM was rewritten
    if (typeof KTComponents !== 'undefined' && typeof KTComponents.init === 'function') {
        KTComponents.init();
    } else if (typeof KTMenu !== 'undefined' && typeof KTMenu.createInstances === 'function') {
        KTMenu.createInstances('[data-kt-menu="true"]');
    } else if (typeof KTMenu !== 'undefined' && typeof KTMenu.init === 'function') {
        KTMenu.init();
    }
};
...
Template.start = function () {
    ...
    Template.convertMetronicMenu();

    Template.connectMenu();

    if (Template.navbarOptions['has_menu_mode_switch'] == '1') {
        let menuTheme = Template.getMenuTheme();
        document.documentElement.setAttribute('data-menu-theme', menuTheme);
        $('#menu_theme_switch').prop('checked', (menuTheme == 'dark'));
        $('#kt_app_body').attr('data-kt-app-layout', (menuTheme == 'dark') ? 'dark-sidebar' : 'light-sidebar');
    }
...
Template.closeRightPanel = function () {
    ...
    if ($('.popover.trigger.click.show').length > 0) {
        ...
        if ($('#adianti_right_panel > [page_name]').length == 1) {
            $('#adianti_right_panel').hide('slide', {
        ...
        $('#adianti_right_panel > [page_name]').last().hide('slide', {
            direction: 'right', complete: function () {
